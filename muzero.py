...
class MuZero:
    ...
    def train(self, log_in_tensorboard=True):
        ...
        # Initialize workers
        print("啟動 training_worker")
        self.training_worker = trainer.Trainer.options(
            num_cpus=0,
            num_gpus=num_gpus_per_worker if self.config.train_on_gpu else 0,
        ).remote(self.checkpoint, self.config)
        print("啟動 shared_storage_worker")
        ...
        print(f"啟動 self_play_workers 數量: {len(self.self_play_workers)}")
    ...
    def logging_loop(self, num_gpus):
        ...
        try:
            while info["training_step"] < self.config.training_steps:
                ...
                counter += 1
                # === 新增：每個 checkpoint interval 時同步存 replay buffer ===
                if self.config.save_model and info["training_step"] % self.config.checkpoint_interval == 0 and info["training_step"] > 0:

                    path = self.config.results_path / "replay_buffer.pkl"
                    # 向 ReplayBuffer worker 發送一個“保存”請求，然後立刻繼續，不等待它完成
                    self.replay_buffer_worker.save_buffer_to_disk.remote(path)
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        # --- MODIFICATION START ---
        # Directly set the game to 'chess' instead of asking the user.
        game_name = "chess"
        print(f"\nWelcome to MuZero! Defaulting to game: {game_name}")
        ...
        while True:
            ...
            if choice == 0:
                ...
                muzero.test(render=True, opponent="self", muzero_player=None,num_gpus=4)
            elif choice == 4:
                muzero.test(render=True, opponent="human", muzero_player=0,num_gpus=0)
            elif choice == 5:
                env = muzero.Game()
                env.reset()
                env.render()
                ...
                while not done:
                    ...
                    if action == -1: # Handle quit action
                        break
