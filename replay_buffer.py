...
class ReplayBuffer:
    ...
    # 在 ReplayBuffer class 內部

    def save_buffer_to_disk(self, path):
        """
        異步地將當前的 buffer 保存到硬碟。
        這個方法會由主循環遠程調用。
        """
        if self.num_played_steps > 0:
            print(f"Replay Buffer: Persisting buffer to {path}...")
            try:
                # 我們只保存核心的 buffer 數據
                pickle.dump(self.buffer, open(path, "wb"))
                print(f"Replay Buffer: Successfully saved buffer to {path}")
            except Exception as e:
                print(f"Replay Buffer: Failed to save buffer. Error: {e}")
