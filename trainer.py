...
class Trainer:
    ...
    def continuous_update_weights(self, replay_buffer, shared_storage):
        ...
        while self.training_step < self.config.training_steps and not ray.get(
            ...
            if last_loss is not None:
                print(f"[TRAIN] Step {self.training_step-1}: Loss: {last_loss:.4f}")
            ...
            if self.training_step % self.config.checkpoint_interval == 0:
                ...
                if self.config.save_model:
                    # 新增：每次存不同檔名
                    checkpoint_name = f"model_step_{self.training_step}.checkpoint"
                    checkpoint_path = str(self.config.results_path / checkpoint_name)
                    shared_storage.save_checkpoint.remote(checkpoint_path)
            shared_storage.set_info.remote(
                {
    ...
    def update_lr(self):
        lr = self.config.lr_init
        for s in self.config.lr_decay_steps:
            if self.training_step >= s:
