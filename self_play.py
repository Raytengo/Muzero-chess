...
class SelfPlay:
    ...
    def continuous_self_play(self, shared_storage, replay_buffer, test_mode=False):
        print("SelfPlay worker 進入 continuous_self_play")
        while ray.get(
    ...
    def play_game(
        ...
        try:
            ...
            print(f">>>> [SelfPlay] play_game: self.game.reset() 發生錯誤: {e}")
            raise e
        ...
        with torch.no_grad():
            while (
                ...
                try:
                    ...
                    player = self.game.to_play_history[-1] if hasattr(self.game, 'to_play_history') and self.game.to_play_history else self.game.to_play()
                    color = '白' if player == 0 else '黑'
                    move_uci = self.game.action_to_string(action)
                    print(f"Step {len(game_history.action_history)}: Player {color}, Move: {move_uci}, Reward: {reward}")
                except Exception as e:
                    print(f">>>> [SelfPlay] play_game: self.game.step() 發生錯誤: {e}")
                    raise e
